...
export async function signPDF(
  ...
  pagesToSign: { pageIndex: number; x: number; y: number; width: number; height: number }[]
): Promise<Uint8Array> {
  ...
  for (const { pageIndex, x, y, width, height } of pagesToSign) {
...
export async function addPageNumbers(
  ...
  options: {
    ...
    format: "n" | "page-n" | "page-n-of-m" | "n-of-m";
    startNumber: number;
    fontSize?: number;
    margin?: number;
  }
  ...
  pages.forEach((page, index) => {
    ...
    switch (options.format) {
      ...
      case "page-n":
        ...
        break;
      case "n-of-m":
        text = `${pageNum} of ${totalPages + startNum - 1}`;
        break;
      case "page-n-of-m":
        text = `Page ${pageNum} of ${totalPages + startNum - 1}`;
        break;
    }
...
export async function getPDFMetadata(file: File): Promise<PDFMetadata> {
  ...
  return {
    ...
    keywords: pdf.getKeywords() ? pdf.getKeywords()!.split(";") : [], // pdf-lib returns string, we might want array? Actually pdf-lib getKeywords returns string usually separated by something?
    // Wait, pdf.getKeywords() returns string | undefined.
    // Let's assume standard behavior. Often it's space or semicolon separated.
    // For simplicity, let's keep it as is or split.
...
export async function htmlToPDF(
  ...
  try {
    ...
    const canvas = await html2canvas(bodyElement, {
      ...
      // @ts-ignore - html2canvas v1.x supports letterRendering but types might be outdated
